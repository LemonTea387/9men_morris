...
void MillObserver::Notify(Tile* tileAffected) {
    ...
    for (const auto index: millLookup[coord]) {
        const auto a = m_Gameboard->GetTile(mills[index][0].first, mills[index][0].second);
        const auto b = m_Gameboard->GetTile(mills[index][1].first, mills[index][1].second);
        const auto c = m_Gameboard->GetTile(mills[index][2].first, mills[index][2].second);
